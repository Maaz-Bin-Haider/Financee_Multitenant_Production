...
The comprehensive suite is `tests/suite/` (run `python tests/suite/run_all.py` in the container; see `tests/suite/README.md` and `tests/suite/RESULTS.md`). It covers **every** domain and **every** report against all tenants, asserting real accounting invariants (double-entry balance, party balances, COGS, stock/serial coherence), and supports an `XFAIL`/`known_bug` channel. Latest run: all modules pass, 0 XFAIL.

The suite surfaced genuine **tenant schema drift** — idempotent `tenancy/sql/` patches applied to one tenant but not the other. Most were healed by `tenancy/sql/fix_tenant_drift.sql` (tenant schema version 4): the `create_purchase_return` in-stock guard, the ambiguous `item_transaction_history(text)` overload, and `get_item_names_like`. Still **deferred**: the cash-party feature (`is_cash` column, `get_cash_party_id`) is absent on `tenant_company_1` — porting it risks regressing the integrity fixes, so the suite feature-detects it. When you change tenant SQL, apply it to *all* tenants (`apply_sql_all_tenants`) to avoid widening drift.
