...
--         * tenancy registry tables (tenancy_company, tenancy_membership)
...
--    3. A registry row linking the example company to its schema.
--
...
-- ----------------------------------------------------------------------------
-- 1b. Tenancy registry tables (public, shared).  Schemas here MUST match the
--     tenancy app's 0001_initial migration exactly.  The migration row below
--     marks that migration applied so `manage.py migrate` will not recreate it.
-- ----------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS public.tenancy_company (
...
CREATE TABLE IF NOT EXISTS public.tenancy_membership (
    id          bigint GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    user_id     integer NOT NULL UNIQUE
                  REFERENCES public.auth_user(id) ON DELETE CASCADE,
    company_id  bigint  NOT NULL
                  REFERENCES public.tenancy_company(id) ON DELETE CASCADE,
    created_at  timestamptz NOT NULL DEFAULT now()
);
CREATE INDEX IF NOT EXISTS tenancy_membership_company_id_idx
    ON public.tenancy_membership(company_id);

INSERT INTO public.django_migrations (app, name, applied)
VALUES ('tenancy', '0001_initial', now())
ON CONFLICT DO NOTHING;
...
-- ----------------------------------------------------------------------------
INSERT INTO public.tenancy_company (name, schema_name, is_active)
VALUES ('Company One', 'tenant_company_1', true)
