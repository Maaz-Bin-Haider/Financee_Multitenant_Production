-- ============================================================================
--  build_multitenant_db.sql
--  ---------------------------------------------------------------------------
--  ONE-SHOT build of the schema-based MULTI-TENANT database.
--  Target: an EMPTY PostgreSQL 16 database.
--
--      createdb mydb
--      psql -v ON_ERROR_STOP=1 -d mydb -f build_multitenant_db.sql
--      python manage.py migrate     # runs `authentication` permission migrations
--
--  What this script creates:
--    1. PUBLIC (shared) schema:
--         * Django / auth framework tables (auth_*, django_*)
--         * framework migration history seed
--         (the tenancy registry tables are created by `migrate`, not here)
--    2. An example TENANT schema `tenant_company_1` containing every business
--       object (tables / functions / views / triggers / sequences) + the
--       Chart-of-Accounts seed, all isolated inside that schema.
--    3. The registry row linking the example company to its schema is created
--       after `migrate`, by `manage.py register_bootstrap_tenant`.
--
--  Add more tenants at runtime through the Django admin (Companies) or:
--      python manage.py provision_tenant "Company Two"
-- ============================================================================
...
-- ----------------------------------------------------------------------------
-- 1a. SHARED Django / auth framework objects (live in public)
-- ----------------------------------------------------------------------------
--
...
--
-- Mark Django FRAMEWORK migrations as applied (the schema they build is already
-- created above). The 15 'authentication' permission migrations are intentionally
-- left UNSEEDED so `python manage.py migrate` runs them and creates the permissions.
--
...
-- ----------------------------------------------------------------------------
-- 1b. Tenancy registry tables: intentionally NOT created here.
--
--     This script used to create tenancy_company/tenancy_membership and seed
--     ('tenancy','0001_initial') as applied. That made the serial-only squashed
--     replacement migration permanently *partially* applied, and Django only
--     uses a replacement when ALL or NONE of the migrations it replaces are
--     applied. It therefore discarded the replacement, replayed the original
--     chain, and 0005_company_inventory_mode recreated the retired
--     `inventory_mode` column on every fresh install.
--
--     `manage.py migrate` now owns the entire public tenancy schema, so the
--     migrations are the single source of truth for its shape. The example
--     tenant schema built below is registered afterwards by the entrypoint:
--         python manage.py register_bootstrap_tenant
--     See SERIAL_ONLY_REMOVAL_PLAN.md checkpoint 4A.
-- ----------------------------------------------------------------------------

-- ----------------------------------------------------------------------------
-- 2. Example tenant schema: tenant_company_1
-- ----------------------------------------------------------------------------
...
-- ============================================================================
-- Tenant drift heal — folded from tenancy/sql/fix_tenant_drift.sql
-- ============================================================================
-- ----------------------------------------------------------------------------
...
-- ============================================================================
-- Cash-party feature (from fix_cash_party_port.sql; v5),
-- including its invoice-description prerequisite and pre-flag journal backfill.
-- ============================================================================
...
-- ----------------------------------------------------------------------------
-- 3. Register the example tenant in the registry
--
--     Deferred: the registry tables are created by `manage.py migrate`, which
--     runs after this script. The container entrypoint registers this schema
--     immediately afterwards with `manage.py register_bootstrap_tenant`, which
--     refuses to act on any database that already contains a company.
-- ----------------------------------------------------------------------------
