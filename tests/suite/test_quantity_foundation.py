...
def main():
    ...
    chk("quantity required version is one", quantity.required_version == 1)
    ...
    try:
        ...
        chk("metadata family/version/currency matches public company",
            all(row == ("quantity", 1, "PKR") for row in metadata), metadata)
        ...
        chk("document sequences seeded exactly once",
            all(q(c.schema_name, "SELECT count(*) FROM document_sequences")[0][0] == 10
                for c in companies))
        chk("foundation seed registered exactly once",
            all(q(c.schema_name, "SELECT count(*) FROM quantity_seed_registry")[0][0] == 1
                for c in companies))
        ...
        chk("quantity hardening is idempotent",
            all(q(c.schema_name, "SELECT count(*) FROM document_sequences")[0][0] == 10
                for c in companies)
            and all(q(c.schema_name, "SELECT count(*) FROM quantity_seed_registry")[0][0] == 1
                    for c in companies))
