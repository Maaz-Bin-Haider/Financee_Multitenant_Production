...
def main():
    ...
    chk("quantity required version includes warehouse foundation",
        quantity.required_version == 4)
    ...
    try:
        ...
        chk("metadata family/version/currency matches public company",
            all(row == ("quantity", 4, "PKR") for row in metadata), metadata)
        ...
        chk("document sequences seeded exactly once",
            all(q(c.schema_name, "SELECT count(*) FROM document_sequences")[0][0] == 10
                for c in companies))
        chk("all four foundation phase seeds registered exactly once",
            all(q(c.schema_name, """
                SELECT count(*), count(DISTINCT seed_key)
                  FROM quantity_seed_registry
            """)[0] == (4, 4) for c in companies))
