...
CREATE TABLE chart_of_accounts (
    ...
    CONSTRAINT chart_of_accounts_name_unique UNIQUE (account_name),
...
CREATE TABLE journal_entries (
    ...
    CONSTRAINT journal_entries_reversal_shape_check CHECK (
        ...
        OR
        (original_journal_id IS NOT NULL AND status = 'posted')
    ),
...
CREATE INDEX journal_entries_date_idx ON journal_entries(entry_date, journal_id);
...
CREATE FUNCTION quantity_assert_journal_balanced()
RETURNS trigger
LANGUAGE plpgsql
AS $$
DECLARE
...
CREATE FUNCTION quantity_protect_posted_journal()
RETURNS trigger
LANGUAGE plpgsql
AS $$
BEGIN
...
CREATE VIEW quantity_trial_balance AS
SELECT
    coa.account_id,
    coa.account_code,
    coa.account_name,
...
CREATE FUNCTION quantity_account_lookup(search_text text DEFAULT NULL)
RETURNS TABLE (
    account_id bigint,
...
CREATE FUNCTION quantity_next_document_number(p_document_type text)
RETURNS text
LANGUAGE plpgsql
AS $$
DECLARE
...
BEGIN
    UPDATE document_sequences
     ...
     RETURNING prefix, next_number - 1
          INTO selected_prefix, selected_number;
    IF NOT FOUND THEN
...
CREATE FUNCTION quantity_post_journal(
    p_entry_date date,
    p_description text,
    p_source_document_type text,
    p_source_document_id bigint,
    p_source_document_number text,
...
CREATE FUNCTION quantity_reverse_journal(
    p_journal_id bigint,
...
BEGIN
    ...
    IF original.status <> 'posted'
       OR original.original_journal_id IS NOT NULL
    ...
    UPDATE journal_entries
       SET original_journal_id = p_journal_id
     WHERE journal_id = new_journal_id;
    UPDATE journal_entries
       SET status = 'reversed',
           reversal_journal_id = new_journal_id,
